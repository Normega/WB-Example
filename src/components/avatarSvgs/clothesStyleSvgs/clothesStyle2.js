export const ClothesStyle2 = ({ shirtColor }) => {
  return (
    <svg>
      <path
        style={{
          display: "inline",
          fill: shirtColor,
          fillOpacity: 0.979661,
          strokeWidth: 0.389336,
        }}
        d="M54.686 175.401C8.689 186.543 10.917 207.51 10.917 207.51c70.47-.345 115.25 1 120.66-.184-13.045-31.765-45.535-31.48-45.535-31.48-13.59 6.604-15.758 7.703-31.356-.444z"
        transform="translate(5, -64) scale(0.85, 1)"
      />
      <path
        style={{
          fill: "#fff",
          fillOpacity: 0.6,
          strokeWidth: 0.264583,
        }}
        d="M54.917 175.561c-8.865 11.027 6.054 31.783 6.054 31.783h19.675s-14.702 24.648 0 0 5.621-31.999 5.621-31.999 13.621-4.54 0 0c-13.62 4.54-31.35.216-31.35.216z"
        transform="translate(5, -64) scale(0.85, 1)"
      />
      <path
        style={{
          fill: "#fff",
          fillOpacity: 0.4,
          strokeWidth: 0.264583,
        }}
        d="m55.35 175.561-11.46 7.568 4.108 4.973-4.324 3.675 10.162 15.567h7.351s16.216 24.432 0 0c-16.216-24.431-5.838-31.783-5.838-31.783z"
        transform="translate(5, -64) scale(0.85, 1)"
      />
      <path
        style={{
          fill: "#fff",
          fillOpacity: 0.4,
          strokeWidth: 0.264583,
        }}
        d="m86 175.446 11.46 7.568-4.108 4.973 4.324 3.675-10.162 15.567h-7.351s-16.216 24.432 0 0C96.378 182.798 86 175.446 86 175.446z"
        transform="translate(5, -64) scale(0.85, 1)"
      />
      <path
        style={{
          fill: shirtColor,
          fillOpacity: 1,
          strokeWidth: 0.264583,
        }}
        d="m64.427 177.253-2.023 5.765 4.006 6.078-2.095 18.187h11.473L72.22 188.66l3.902-5.405-2.065-6.002z"
        transform="translate(5, -64) scale(0.85, 1)"
      />
    </svg>
  );
};
